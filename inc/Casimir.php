...
class Casimir {
  ...
  function __construct() {
    ...
    if ($this->db->connect_error) $this->error500((_('Could not connect to database')));
    ...
    if (defined('ACCESS_KEY') && ACCESS_KEY != '' && ACCESS_KEY != $this->access_key) {
      ...
      if (isset($_POST['long'])) {
        if ((array_key_exists("site", $_POST)) && (trim($_POST["site"]) != "") && (in_array($_POST["site"], $this->sites)))
          $site = $_POST["site"];
        list($this->ok, $this->short, $this->msg) = $this->addUrl($_POST['long'], $site, isset($_POST['short']) && !is_null($_POST['short']) && $_POST['short'] != 'null' ? $_POST['short'] : '', $api);
      } elseif (isset($_GET['long'])) {
        list($this->ok, $this->short, $this->msg) = $this->addUrl($_GET['long'], $site, isset($_GET['short']) && !is_null($_GET['short']) && $_GET['short'] != 'null' ? $_GET['short'] : '', $api);
      }
    }
  }
  ...
  function getShort($site, $long) {
    $q = 'SELECT short_url FROM casimir WHERE site = "'.trim($this->db->real_escape_string($site)).'" AND long_url="'.trim($this->db->real_escape_string($long)).'" ORDER BY creation_date DESC LIMIT 0,1';
  ...
  function getLong($site, $short) {
    $q = 'SELECT long_url FROM casimir WHERE site="'.trim($this->db->real_escape_string($site)).'" AND short_url="'.trim($this->db->real_escape_string($short)).'"';
    $result = $this->db->query($q);
  ...
  function addUrl($long, $site, $short = '', $api = false) {
    ...
    $site = trim($this->db->real_escape_string($site));
    if ($short != '') {
